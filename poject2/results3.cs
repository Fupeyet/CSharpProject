...
namespace poject2
{
    public partial class results3 : MaterialForm
    {
        ...
        private void results3_Load(object sender, EventArgs e)
        {
            NpgsqlConnection con = new NpgsqlConnection(connectionString);
            con.Open();
            NpgsqlCommand command1 = new NpgsqlCommand("SELECT result2 FROM results3 WHERE login=@uP", con);
            command1.Parameters.AddWithValue("@uP", Log.login);

            NpgsqlCommand command2 = new NpgsqlCommand("SELECT result1 FROM results3 WHERE login=@uP", con);
            command2.Parameters.AddWithValue("@uP", Log.login);

            NpgsqlCommand command3 = new NpgsqlCommand("SELECT result3 FROM results3 WHERE login=@uP", con);
